 ...
 <table  style=" width:100%" cellpadding="5" class="widefat post sub_table" >
    <thead>
     <tr>
		...
		$catinfo = $wpdb->get_results("SELECT t.*  FROM $wpdb->terms AS t INNER JOIN $wpdb->term_taxonomy AS tt ON tt.term_id = t.term_id WHERE tt.taxonomy ='".$post_type[1]."'");
		}
		...
		$path= get_template_directory_uri().'/monetize/upload/index.php?img=term_icon'.$term_id.'&nonce=mktnonce';
		?>
      <tr>
        <td><?php echo $name;?> </td>
...
$wpdb->query("update $wpdb->terms set term_price = \"$cprice\",term_icon=\"$term_icon\"  where term_id=\"$catid \"");
	...
	$post_type = explode(",",$_REQUEST['post_type']);
get_wp_category_checklist($post_type[1],'');
}
?>
