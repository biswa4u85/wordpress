...
function after_setup_theme_option_tree_setup()
{
	...
	if(isset($_REQUEST['page'])=='templatic_wp_admin_menu')
	{
		// drop table
		...
		$wpdb->query($sql);
		$xmlfilename = apply_filters('templ_theme_options_xmlpath_filter',TT_ADMIN_FOLDER_PATH.'theme_options/theme-options.xml');
...
{
	if(isset($_REQUEST['ajaxact'])=='save_options')
	{
		if($_POST)
		{
		...
		exit;
	}elseif(isset($_REQUEST['ajaxact'])=='reset_options')
	{
